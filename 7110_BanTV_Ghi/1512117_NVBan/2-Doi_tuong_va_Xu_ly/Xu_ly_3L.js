    ...
    for(var i=0; i< danh_sach.length; i++){
       ...
       // alert(Danh_sach_Ban_hang.length);
        ...
        for(var j=0;j<Danh_sach_Ban_hang.length;j++)
        {
          ...
          Doanh_Thu=Doanh_Thu+parseInt(Danh_sach_Ban_hang[i].getAttribute("Tien"));
        }
...
function Tao_phieu_ban()
{
    //set lại body rỗng 
    Xoa_Tat_ca_childNodes_Element(CUA_SO_KET_QUA)
    //Sửa lại tiêu đề
    document.getElementById("quan_ly_nhap_hang").innerHTML = "BÁN HÀNG THEO PHIẾU BÁN"
    //Tạo form cho phiếu bán
    var Phieu_ban=document.createElement("div")
    Phieu_ban.innerHTML=`<div class="row">
  ...
  <div class="row">
    ...
    <div class="form-inline">
      ...
      <input type="text" class="form-control" id="hoten" placeholder="Nguyễn Minh Đồng" value="mm">
    </div>
    <br>
    </div>
  </div>
  ...
  <br>
  <div class="row">
  <div class="col-sm-8"><button type="button" class="btn btn-primary" onclick="Xu_li_Ban_hang()">Submit</button></div>
</div>
  `
    CUA_SO_KET_QUA.appendChild(Phieu_ban)
}
function Xu_ly_phieu_ban()
{
...
function Xu_li_Ban_hang()
{
    ...
    var Tham_so = `ReqCode=NV_Ban&Ten=${Ten_tivi}&Ngay=${Ngay}&Don_gia=${Don_gia}&So_luong=${So_luong}&Tien=${Tien}`;
    var Dia_chi_Xu_ly = `${Dia_chi_Dich_vu}?${Tham_so}`;
...
function Tao_phieu_Ban_hang()
{
    var Nut_phieu_ban = document.createElement("button")
    Nut_phieu_ban.innerHTML = "PHIẾU BÁN"
    Nut_phieu_ban.id = "btn_tao_phieu_ban"
    Nut_phieu_ban.onclick = function() {Tao_phieu_ban()}
    return Nut_phieu_ban
}
